...
class ResearchPDFHandler:
    ...
    def _create_new_vectorstore(self):
        ...
        try:
            ...
            logger.info("Loading PDF document for processing")
            ...
            logger.info("Creating embeddings with multilingual E5 model")
            embeddings = HuggingFaceEmbeddings(
                model_name="intfloat/multilingual-e5-small",
                model_kwargs={'device': 'cpu'},
                encode_kwargs={'normalize_embeddings': True}
            ...
            logger.error(f"Error creating vector store from PDF: {e}")
    ...
    def search_research(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error searching research document: {e}")
    ...
    def initialize_research_handler():
        ...
        try:
            ...
            if os.path.exists(research_pdf_path):
                ...
                logger.info("Research handler initialized successfully")
                return research_handler
            else:
                raise FileNotFoundError(f"PDF de recherche introuvable à : {research_pdf_path}")
        except Exception as e:
            logger.error(f"Failed to initialize research handler: {e}")
...
def query_research_document(query: str, max_results: int = 5) -> str:
    ...
    if _research_handler is None:
        try:
            ...
            logger.error(f"Error initializing research handler: {e}")
    ...
    try:
        ...
        logger.error(f"Error accessing research document: {e}")
